...
class Server
{
     ...
     * @param callable   $authFunc
     */
    public function __construct($request = '', callable $authFunc = null)
    {
        if ( $authFunc ) {
            if ( ! call_user_func( $authFunc, $this->getUsername(), $this->getPassword() ) ) {
     ...
     *
     * @access public
     * @param  string   $header   Header name1
     * @return Server
     */
    ...
    public function executeMethod($class, $method, $params)
    {
        ...
        if (! empty($this->before)) {
            if (is_callable($this->before)) {
                call_user_func_array($this->before, array($this->getUsername(), $this->getPassword(), get_class($class), $method, $params));
            }
